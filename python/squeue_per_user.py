    ...
    if job_dict:
        for jobid, value in sorted(job_dict.items()):
            if value["account"] not in users:
                users.append(value["user_id"])
    return unique(users)

def get_squeue(job_dict,partition_key,uid_key,key=None):
    RUNNING = 0
    PENDING = 0
    BOOT_FAIL = 0
    CANCELLED = 0
    ...
    for jobid in job_dict:
        if partition_key in job_dict[jobid]["partition"]:
            if str(uid_key) in str(job_dict[jobid]["user_id"]):
            ...
            try:
                ...
                if len(jobs) > 0:
                    ...
                    for partition in partitions:
                            for uid in users:
                                num_jobs = get_squeue(jobs,partition,uid)
                                num_cpus = get_squeue(jobs,partition,uid,'num_cpus')
                                print("slurm,metric=num_jobs,hostname={0},partition={1},username={2} {3}".format(socket.gethostname(),partition,getpwuid(uid).pw_name,num_jobs))
                                print("slurm,metric=num_cpus,hostname={0},partition={1},username={2} {3}".format(socket.gethostname(),partition,getpwuid(uid).pw_name,num_cpus))
                else:
                    print("No jobs found !")
